# interlude_2.rb — "half-time gallop"
# 140 BPM | ~2 min | Em | heavy half-time drums + fuzz wall
#
# run_file "/Users/logan/Projects/chiptune/set6/interlude_2.rb"
...
# dark noise wall
in_thread do
  sample dark_noise, amp: 0.55, attack: 2.0, sustain: 118.0, release: 2.0
end
    ...
    with_fx :bitcrusher, bits: 4, sample_rate: 0.18 do
      ...
      [[:e1, 0.93, 0.0, 0.55], [:e2, 0.90, -0.8, 0.45],
       [:g2, 0.92, 0.6, 0.35], [:b2, 0.89, -0.4, 0.32]].each do |pitch, rate, pan, amp|
        play pitch, sustain: 122.0, release: 6.0,
...
in_thread do
  with_fx :distortion, distort: 0.4 do
    10.times do
      4.times do
        sample :bd_haus, amp: 2.5, rate: 0.92    # beat 1
        sleep 1
        sleep 1                                    # beat 2
        sample :bd_haus, amp: 1.8, rate: 0.95    # beat 3 (gallop 1)
        sleep 0.5
        sample :bd_haus, amp: 2.0, rate: 0.93    # beat 3.5 (gallop 2)
        sleep 0.5
        sample :sn_dolf, amp: 2.8, rate: 0.78    # beat 4 (half-time snare)
        sample gpercs.choose, amp: rrand(0.4, 0.9), rate: rrand(0.7, 1.4) if one_in(2)
        sleep 1
        sleep 1                                    # beat 5
        sleep 1                                    # beat 6
        sample :bd_haus, amp: 2.3, rate: 0.90    # beat 7 (gallop 1)
        sleep 0.5
        sample :bd_haus, amp: 1.9, rate: 0.94    # beat 7.5 (gallop 2)
        sleep 0.5
        sample :sn_dolf, amp: 3.0, rate: 0.75    # beat 8 (heavy half-time snare)
