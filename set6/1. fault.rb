# fault.rb — set6: +20% length — adds second verse (t=6,7) before final chorus
# 175 BPM  |  B minor  |  Bm – G – D – A
#
# t=0   — breakdown:      whole-note bass, beat 1 kick, snare on 3, crash, no hats
# t=1,2 — verse I:        walk bass, 4-on-floor kick, ska hats, arp
# t=3,4 — chorus:         stabs + gallop bass, double kick, 16th hats
# t=5   — glitch breakdown: whole-note bass, crash, heavy glitch texture
# t=6,7 — verse II:       walk bass, 4-on-floor kick, ska hats, arp (new section)
# t=8   — final chorus:   arp + stabs together, blast kick
# t=9   — breakdown:      stripped close, mirrors t=0
#
# run_file "/Users/logan/Projects/chiptune/set6/1. fault.rb"
...
# ── Ascending arp (verse I + verse II + final chorus) ─────────────────────────
ARP_UP = [
...
live_loop :arp, sync: :conductor do
  t = tick
  stop if t >= SONG_END
  if t.between?(1, 2) || t.between?(6, 7) || t == 8
...
live_loop :chip_bass, sync: :conductor do
  ...
  with_fx :distortion, distort: 0.6 do
    ...
    elsif t.between?(1, 2) || t.between?(6, 7)
