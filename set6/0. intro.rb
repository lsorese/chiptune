# intro.rb — set6: +20% length — 2 extra chord pairs (C + G) extend bass/melody to 32 beats
# 60 BPM  |  A minor  |  somber chiptune intro → dissonant cluster → first track
#
# 0:00–0:32 — bass + two melody channels (left / right), sustained
#             Am – F – C – Em – Am – F – C – G
# 0:32–0:38 — chromatic sweep, one pitch at a time, each in 8 octaves, noise blast
#
# run_file "/Users/logan/Projects/chiptune/set6/0. intro.rb"
...
with_fx :reverb, room: 0.3, mix: 0.12 do
  ...
  # ── 32 beats: bass + sustained stereo melody ────────────────────────────────
  ...
  # left channel: e4→a3→g3→a3→a4→e4→d4→e4
  in_thread do
    use_synth :pulse
    [[:e4, 6], [:a3, 2], [:g3, 6], [:a3, 2], [:a4, 6], [:e4, 2], [:d4, 4], [:e4, 4]].each do |n, dur|
  ...
  # right channel: d4→c4→e3→b3→g4→f4→e4→d4 (starts 2 beats in)
  in_thread do
    sleep 2
    use_synth :pulse
    [[:d4, 2], [:c4, 6], [:e3, 2], [:b3, 6], [:g4, 2], [:f4, 4], [:e4, 4], [:d4, 4]].each do |n, dur|
      play n, pan: 1, amp: 0.58, attack: 0.05, sustain: dur - 0.1, release: 0.05,
              pulse_width: 0.25, cutoff: 78
      sleep dur
    end
