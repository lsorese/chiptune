...
# sec 1   — break:    half-time kick (2-on-floor), no hats, slow bass,
...
#
# run_file "/Users/logan/Projects/chiptune/set4/5. wire.rb"
...
# ── Descending stabs ──────────────────────────────────────────────────────────
# sec 1: rapid burst + long gap, ring mod at mid freq
#   2 passes × 4 chords × (4×0.125 + 0.5 + 4×0.125 + 2.5) = 32 beats ✓
# sec 3: full-speed stabs + ring mod at high freq
...
# ── Chip bass ─────────────────────────────────────────────────────────────────
# sec 0,2: walk (2×4×8×0.5 = 32) ✓
# sec 1:   half-time — root+walk, 2 beats each (2×4×(2+2) = 32) ✓
# sec 3:   gallop (2×4×4×1 = 32) ✓
...
# ── Kick ──────────────────────────────────────────────────────────────────────
# sec 0,2: 4-on-floor (32×sleep1 = 32) ✓
# sec 1:   2-on-floor/half-time (16×sleep2 = 32) ✓
...
live_loop :kick, sync: :conductor do
  ...
  when 1
    16.times do
      sample :bd_haus, amp: 2.5; sleep 2
    end
  when 3
    64.times do
...
live_loop :snare, sync: :conductor do
  ...
  if [0, 2].include?(sec)
    ...
    8.times do
      sleep 2
      sample :sn_dolf, amp: 2.8, rate: 0.82
      sleep 2
    end
  else
    8.times do
