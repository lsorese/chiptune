# interlude_3.rb — "doom march"  (set7: trimmed to ~48s)
# 120 BPM | Ab | ultra-slow doom kick + noise swells
# kit: bd_fat / sn_dub
#
# run_file "/Users/logan/Projects/chiptune/set7/interlude_3.rb"
...
# three noise layers
in_thread do
  sample noise_loop, amp: 0.50, attack: 4.0, sustain: 42.0, release: 4.0
  sample dark_noise, amp: 0.55, attack: 2.0, sustain: 44.0, release: 2.0
  sample ambi_buzz,  amp: 0.30, attack: 8.0, sustain: 38.0, release: 8.0
end
...
in_thread do
  with_fx :bitcrusher, bits: 3, sample_rate: 0.08 do
    with_fx :distortion, distort: 0.98 do
      use_synth :chipbass
      play :ab0, sustain: 46.0, release: 6.0, amp: 2.8, cutoff: 52
      play :eb1, sustain: 46.0, release: 6.0, amp: 1.4, cutoff: 46
    end
  end
  sleep 96
end
...
in_thread do
  with_fx :distortion, distort: 0.7 do
    3.times do
      4.times do
        sample :bd_fat, amp: 3.2, rate: 0.60    # massive slow kick on beat 1
        sleep 1
        sleep 1
        sample :sn_dub, amp: 3.0, rate: 0.68    # heavy snare on beat 3
        sample gpercs.choose, amp: rrand(0.5, 1.0), rate: rrand(0.3, 0.9) if one_in(2)
        sleep 1
        sample :bd_fat, amp: 2.4, rate: 0.65 if one_in(2)   # optional beat 4 kick
        sleep 1
        # 4-beat bar total: 1+1+1+1 = 4 ✓; 4 bars × 4 beats = 16... need 32 per block
        # Actually 4.times × 8 beats = 32 ✓ — need 8 beats per inner loop
        sample :bd_fat, amp: 2.8, rate: 0.62    # beat 5
        sleep 1
        sleep 1
        sample :sn_dub, amp: 3.2, rate: 0.70    # beat 7 (heavy)
